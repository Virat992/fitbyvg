...
export default function Welcome({ onNext, onBack }) {
  ...
  return (
    <div className="w-full max-w-md md:max-w-lg mx-auto h-dvh flex flex-col bg-gradient-to-b from-cyan-50 via-white to-cyan-100">
      {/* Header */}
      <div className="px-4 pt-6 md:pt-8 mb-8 flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900">
          Welcome
        </h1>
        <button
          onClick={handleLogout}
          className="text-sm md:text-base font-semibold text-cyan-600 cursor-pointer hover:font-bold"
        >
          Logout
        </button>
      </div>
      ...
      <div className="flex-1 px-4 md:px-6 overflow-y-auto">
        ...
        <p className="text-gray-600 text-base md:text-lg flex items-center gap-1 mb-6">
          We'd like to get to know you. <span>🙂</span>
        </p>

        {/* Input Section */}
        <div>
          <label className="block text-gray-600 text-sm md:text-base mb-3">
          ...
          <input
            ...
            className="w-full px-4 py-4 md:py-5 text-lg md:text-xl border border-gray-200 rounded-xl focus:outline-none focus:border-cyan-600 focus:ring-1 focus:ring-cyan-600 bg-gray-50"
            autoFocus
          />
        </div>
      </div>
